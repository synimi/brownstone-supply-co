...
const OUT = path.join(ROOT, "product");
const pyPath = path.join(__dirname, "generate_product_pages.py");

const py = fs.readFileSync(pyPath, "utf8");
// Match closing `]` before def main (blank lines between may vary)
const endMarkerMatch = py.match(/\n\]\r?\n(?:\r?\n)+\s*def main\s*\(/);
const start = py.indexOf("PRODUCTS = [");
const end = endMarkerMatch ? endMarkerMatch.index : -1;
if (start === -1 || end === -1) {
  throw new Error("Could not parse PRODUCTS from generate_product_pages.py");
}
// `end` is the index of `\n` before `]`; slice end is exclusive, so +2 includes `]`
const arrStr = py.slice(start + 11, end + 2).trim();
...
const STICKY_NAV = `<div class="sticky-nav" aria-hidden="true">
    <div class="container">
      <a class="brand" href="../index.html#home">
        <img src="../logo.svg" alt="Brownstone Supply Co" class="brand-logo">
        <div>
...
const HERO_NAV = `<div class="hero-nav">
      <div class="container">
        <a class="brand" href="../index.html#home">
          <img src="../logo.svg" alt="Brownstone Supply Co" class="brand-logo">
          <div>
...
const MOBILE_MENU = `<div class="mobile-menu" id="mobile-menu" hidden>
...
const SHARED_DELIVERY = `<section class="section-light product-detail-band" id="delivery">
      <div class="container">
...
const SHARED_PROCESS = `<section class="process product-detail-process" id="process">
      <div class="container">
...
const SHARED_CTA = `<section class="shop-cta">
      <div class="container">
...
const FOOTER = `<footer class="shop-page-footer" id="contact">
      <div class="container">
...
function pageHtml(p) {
  const titleEsc = p.title_html;
  const paras = p.paragraphs.map((x) => `<p>${escapeHtml(x)}</p>`).join("\n            ");
  const meta = escapeHtml(p.meta_desc);
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${titleEsc} | Brownstone Supply Co.</title>
  ...
  <header class="hero hero--page product-detail-page-hero" id="top">
    ...
    <div class="hero-content hero-content--page">
      <div class="container">
        ...
        <h1 class="serif product-detail-page-title">${titleEsc}</h1>
        <p class="product-detail-lead">${escapeHtml(p.lead)}</p>
      </div>
    </div>
  </header>
    ...
    <article class="product-detail-article">
      <div class="container product-detail-layout">
        <div class="product-detail-media">
          <img src="${p.img}" alt="${escapeHtml(p.alt)}" width="900" height="675" loading="eager">
        </div>
...
for (const p of products) {
  const outPath = path.join(OUT, `${p.slug}.html`);
  fs.writeFileSync(outPath, pageHtml(p), "utf8");
